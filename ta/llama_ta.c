...
void malloc_run_state(RunState* s, Config* p, float *flat_buf) {
    int kv_dim = (p->dim * p->n_kv_heads) / p->n_heads;
...
void read_weight(float *model_bin, Config* config, TransformerWeights* weights) {
...
void read_config(float *model_bin, Config* config) {
...
void build_transformer(Transformer *t, float* model_bin) {
